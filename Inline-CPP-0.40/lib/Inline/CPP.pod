...
=item c Function overloading -- This is a similar issue to the template problem.
...
=item g Exceptions: While it's ok to throw an exception using 'throw', Perl
...
The current "smoke test" pass rate seems to be around 90%.  I'm always
working on chipping away at that last 10%.  If you're one of the unfortunate
10% get in touch with me so we can try to figure out what the problem is.
Often progress can be made when we find a failure case we can work on together.

=item b Improvements to the test suite.

=item c Address some of the bugs and deficiences mentioned above.

=item d Binding to unions.

=item e More examples.
