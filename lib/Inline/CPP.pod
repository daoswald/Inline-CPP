...
independent of the package from which Inline::CPP has been invoked.  Consider
...
to structs defined in your code. As is the case with C++, structs and classes
are treated the same, except that a struct's initial scope is public, not
...
Is Inline::CPP is ready for the C++11 standard?  The
short answer to that question is "Mostly, to the extent that your compiler is
C++11 standard compliant."  There are two issues to consider.  First, what is
your compiler capable of (and how to you enable those capabilities), and
second, what does Perl need to care about.
...
You also may need to use the CC flag to specify a newer version of the
...
sense to Inline::CPP anyway. And lambdas are still fine, because they take
place inside of functions, so Inline::CPP doesn't see them anyway.  For now,
