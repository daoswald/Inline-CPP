...
Note: Due to an XS implementation detail, C<ref($b)> will return C<main::Foo> if
the C<NAMESPACE> configuration has been set to an empty string; XS doesn't allow
for its internal C<PACKAGE> specification to be empty.  From a caller's
